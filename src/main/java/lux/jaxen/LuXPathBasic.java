...
public class LuXPathBasic extends LuXPath {
    ...
    public XPathQuery getQuery(Expr expr, Context context) throws JaxenException {        
        if (context instanceof JaxenContext) {
            Query contextQuery = ((JaxenContext)context).getQueryContext().getQuery();
            query = getGenericQuery(expr, context);
            if (contextQuery != null) {
                query = query.combine(new XPathQuery(null, contextQuery, XPathQuery.MINIMAL, ValueType.DOCUMENT), Occur.MUST);
            }
        } else {
        ...
        if (query.isImmutable()) {
            query = new XPathQuery (null, query.getQuery(), query.getFacts(), query.getResultType());
        }
        return query;
    }
    ...
    protected XPathQuery getQuery (BinaryExpr binaryExpr, Context context) throws JaxenException {
        ...
        return lhsQuery.combine (occur, rhsQuery, occur, ValueType.ATOMIC);
    }
    ...
    protected XPathQuery getQuery (UnionExpr unionExpr, Context context) throws JaxenException {
        ...
        return lhsQuery.combine (rhsQuery, Occur.SHOULD);
    }
    ...
    protected XPathQuery getQuery (FunctionCallExpr funcallExpr, Context context) throws JaxenException {
        if (funcallExpr.getPrefix().isEmpty()) {
            ...
            XPathQuery query = new XPathQuery(this, new MatchAllDocsQuery(), XPathQuery.MINIMAL, ValueType.ATOMIC);
            for (Object o : funcallExpr.getParameters()) {
                Expr expr = (Expr) o;
                query = query.combine(Occur.MUST, getQuery (expr, context), Occur.SHOULD, ValueType.ATOMIC);
            }
            return query;
        }
        return XPathQuery.UNINDEXED;
    ...
    protected XPathQuery getQuery (LocationPath locationPath, Context context) throws JaxenException {
        XPathQuery query = XPathQuery.MATCH_ALL;
        ...
        for (Object ostep : steps) {
            ...
            query = query.combine(Occur.MUST, stepQuery, Occur.MUST, stepQuery.getResultType());
            ++istep;
        }
        return query;
    }
    ...
    private XPathQuery getQuery(Step step, Context context, boolean isMinimal) throws JaxenException {
        ...
        if (step instanceof AllNodeStep) {
            if (isMinimal) {
                query = XPathQuery.MATCH_ALL;
            } else {
                query = new XPathQuery (this, new MatchAllDocsQuery(), 0, ValueType.NODE);
            }
        }
            ...
            query = new XPathQuery (this, new MatchAllDocsQuery(), 0, ValueType.TEXT);
        }
        /*
    ...
    private XPathQuery applyPredicates(Predicated predicated, XPathQuery query, Context context) throws JaxenException {
        ...
        for (Object o : predicates) {
            ...
            query = query.combine(Occur.MUST, predicateQuery, Occur.MUST, query.getResultType());
        }
        return query;
    }
    ...
    protected XPathQuery getQuery (NameStep nameStep, Context context, boolean isMinimal) throws JaxenException {
        ...
        if (prefix.isEmpty()) {
            if (localName.equals("*")) {
                return new XPathQuery (this, new MatchAllDocsQuery(), isMinimal ? XPathQuery.MINIMAL : 0, valueType);
            }
            qname = localName;
        }
        ...
        return new XPathQuery (this, new TermQuery (new Term (fieldName, qname)), isMinimal ? XPathQuery.MINIMAL : 0, valueType);
    }
